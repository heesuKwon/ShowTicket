			...
			<div class="detailbox_bot_left" id="tabs">
				...
				<div class="tabs-Num" id="tabs-1">
					...
					<div class="detail_cont">
						...
						<strong>[공연장 정보]</strong><br /> 장소 : ${musical.hallName }<br>
						주소 : ${address.adres }<br> 대표번호 : <span id="phoneNumber">${address.telno }</span><br>
						<br /> <br /> <br />
						<div id="map"
							style="clear: both; width: 800px; height: 400px; position: relative; overflow: visible; margin-left: 66px;"></div>
					</div>
                    ...
                    [D] 버튼 선택시 : 선택된 버튼 클래스 'on' 추가, 대체텍스트 '선택됨' 추가, 짝수 점수 버튼에 클래스 st_r 추가, 디폴트는 화면에 보이지 않는 0점 버튼
						...
						<div class="star_review" id="star_review">
							<fieldset>
								...
								<strong class="blind">평점선택</strong> <span class="my-rating-9"
									style="float: left;"></span> <span class="live-rating"
									id="rating" style="float: left;"></span>
								...
								<div class="write_review">
									...
									<button type="button" id="reviewInsertButton"
										onclick="insertReview()" class="btn btn_reply">댓글입력</button>
								</div>
							</fieldset>
				...
				<div class="tabs-Num" id="tabs-3">
					...
					<div class="detail_cont" style="clear: both;">
						...
						<c:if test="${not empty musical.time }">
							<div class="bx_dummy">
								<em class="info_tit">정기 공연일정</em> <br /> <span class="txt">
									${musical.time } <%-- <c:forTokens var="item" items="${musical.time}" delims=",">${item}<br /></c:forTokens> --%>
						...
						<div class="bx_dummy">
							...
							<c:if test="${not empty musical.urls }">
								<c:forEach items="${musical.urls}" var="f">
									<img src="${f }" alt=""
										style="max-width: 900px; margin-left: 110px;" />
								</c:forEach>
							</c:if>
						</div>
					</div>
		...
		marker.setMap(map); 
			...
			<div class="tabs-Num" id="tabs-4">
				...
				<div class="detail_cont">
					...
					<div class="contents">
						<strong>예매 취소 시 주의사항</strong><br> 티켓 예매 후 7일 이내에 취소 시, 취소수수료가
						없습니다. <br>단, 예매 후 7일 이내라도 취소 시점이 공연일로부터 10일 이내라면 그에 해당하는
						취소수수료가 부과됩니다.<br>예매 당일 자정(12:00) 전에 취소할 경우 예매 수수료가 환불되며, 그
						이후에는 환불되지 않습니다.<br>예매티켓 취소는 아래 안내된 취소가능일 이내에만 할 수 있습니다.
						...
						<p>
							...
							500원을 제외한 나머지 금액이 고객 환불 계좌에 입금됩니다.<br>환불은행, 계좌번호, 예금주는 본인명의로
							정확히 입력 부탁드리며, 취소처리를 접수한 날로부터 3~5일 이내에 환불 받으실 수 있습니다.<br> 또한,
							타인의 계좌를 이용하거나 명의를 도용했을 경우 서비스 이용이 제한될 수 있습니다.<br>신용카드로 결제 시,
							취소수수료 및 우편 발송비를 재승인 후 기존 승인금액을 취소 처리합니다.
						</p>
						<table class="detail_info_tbl mgt15">
							<caption></caption>
							<colgroup>
							...
							<tbody>
								<tr>
									...
									<td>없음</td>
									<td rowspan="7">예매 당일에 취소하는 경우 이외에는<br>예매수수료가 환불되지 않음
										(약관 28조 의거)<br> <br>예매 후 7일 이내라도 취소 시점이 공연일로부터<br>10일
					...
					<div class="contents">
						...
						<p>&nbsp;</p>
						<p>
							<span style="line-height: 1.5;">은행에 따라 23:30에 온라인 입금이 마감되는
								경우가 있으니 참고 부탁드립니다.</span> (단, 외환은행, 수협은 22:40에 온라인 입금이 마감됩니다.)
						</p>
						<p>&nbsp;</p>
						<p>
							무통장 입금의 경우 입금 확인 및 고객 확인을 위한 기간이 필요하므로 이용기간이 제한됩니다. 양해 부탁 드립니다. <br>예매
							당일을 포함하여 그 다음날 23:59(주말 및 공휴일 포함)까지 입금하지 않으실 경우 별도의 통보 없이 예매가
							취소됩니다.
						</p>
					</div>
