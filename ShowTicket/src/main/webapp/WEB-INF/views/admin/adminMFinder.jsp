	...
	<div id="report">
		...
		<table class="table" id="reporttbl">
			  ...
			  <c:if test="${empty list }">
			  	<tr>
			  		<th colspan="5">일치하는 검색 결과가 없습니다.</td>
			  	</tr>
			  </c:if>
		...
		<nav aria-label="Page navigation example" class="mnav_top font">
			...
			<c:set var="s" value="${paging.barStart }"/>
				...
				<c:if test="${barNo ne 1}">
					<li class="page-item">
						<a class="page-link" href="${pageContext.request.contextPath }/admin/adminMemberSearch.do?cPage=${barNo-1}" style="background-color: #F2F2F2; border: 0px;">&lt;</a>
					</li>
				</c:if>
					...
					<c:forEach var="barNo" begin="${barNo }" end="${barEnd }">
						<c:choose>
							...
							<c:when test="${(barNo ne s && cPage eq totalPage) }">
								<li class="page-item disabled">
									<a class="page-link" href="${pageContext.request.contextPath }/admin/adminMemberSearch.do?cPage=${barNo}" style="color: gray; border: 0px;">${barNo }</a>
								</li>
							</c:when>
							<c:otherwise>
								<li class="page-item">
				...
				<c:choose>
					...
					<c:when test="${barEnd > totalPage }">
						<li class="page-item disabled">
							<a class="page-link" href="${pageContext.request.contextPath }/admin/adminMemberSearch.do?cPage=${next}" style="background-color: #F2F2F2; border: 0px;">&gt;</a>
						</li>
					</c:when>
					<c:when test="${!(barNo>totalPage || cPage eq totalPage) }">
						<li class="page-item">
